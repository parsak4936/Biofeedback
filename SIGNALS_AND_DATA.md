...
**Adaptive prominence detector** (`mock` and `real_plux`). NeuroKit2 is the primary R-peak detector; a prominence-based two-pass detector is the fallback if NeuroKit fails on degenerate input. The ECG is bandpassed to the 5-15 Hz QRS band, peaks are scored on prominence (how far a peak stands out from its local surroundings, robust to baseline drift) rather than raw height, and a 300 ms refractory window stops the T-wave that follows each beat from being counted as a second heartbeat. HR is per-beat 60000/RR; RMSSD is computed on a rolling 10 s window of RR intervals.

**NeuroKit2 sliding-window chain** (`mock2` and `real_plux2`). The chain matches the reference `vret_server_v2.py`: `nk.ecg_clean` → `nk.ecg_peaks(correct_artifacts=True)`, then `nk.ecg_rate` over a 30-second trailing window for HR (mean of the per-beat rates) and `nk.hrv_time` over a 60-second trailing window for RMSSD. Values are recomputed every 0.5 seconds and held between updates. RMSSD outside 5-300 ms is rejected as a detector failure rather than reported as a real reading.
