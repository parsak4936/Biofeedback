...
**EMA (exponential moving average).** A lightweight smoothing filter, `new = alpha * raw + (1 - alpha) * previous`. The pipeline **no longer uses EMA** — it was removed per the PDF spec. The PDF reference (vret_server.py) explicitly states "EDA is intentionally NOT smoothed... a display-only EMA was previously applied but contributed nothing to S_t and was visually indistinguishable from raw, so it has been removed." HR and RMSSD are already zero-order-hold between 0.5 s recomputes by the data source, so adding an EMA on top would over-smooth them. The term is kept in this glossary only for historical context if you encounter it in older recordings or comments.
...
**Thresholds (mild and high).** Two cutoffs **centred on the resting S_t mean**, not on zero. Mild is `mean_baseline + 1.28 · sigma_baseline` (true 90th-percentile z), high is `mean_baseline + 2.33 · sigma_baseline` (true 99th-percentile z). Centering on `mean_baseline` matters because resting S_t is generally not zero — measuring from zero would flag a calm participant whose resting drift is slightly positive (PDF §4 Bug 6). Both `mean_baseline` and `sigma_baseline` are frozen at the 120 s baseline lock and never adapt during the session.
...
**Live transcript.** The standalone `live_log_*.txt` file was removed in the folder-consolidation refactor. The per-second live summary now just echoes to stdout (operator's terminal) and the per-tick clinical record lives in `samples.csv` inside the session folder. If you need a paste-friendly transcript after the fact, regenerate it from `samples.csv` with `session_review.py`.
